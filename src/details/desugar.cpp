...
auto ForStatement::desugar() const -> DoStatement {
    ...
    auto error_call = Statement(FunctionCall(
        Prefix(VariableDeclarator(Identifier("error", le_range))), nullopt,
    ...
    auto if_condition = UnaryOperation(
        ...
        Expression(BinaryOperation(
            ...
            Expression(BinaryOperation(limit_exp, BinOpEnum::AND, step_exp, le_range)), le_range)),
        le_range);

    /*2*/ auto if_statement = IfStatement(Expression(if_condition), if_body, le_range);

    /*3*/ VariableDeclaration vd1 = VariableDeclaration(
        false, std::vector<VariableDeclarator>{VariableDeclarator(id_var)},
        std::vector<Expression>{
            Expression(BinaryOperation(var_exp, BinOpEnum::SUB, step_exp, le_range))},
        le_range);

    /*W1*/ VariableDeclaration vd2 = VariableDeclaration(
        false, std::vector<VariableDeclarator>{VariableDeclarator(id_var)},
        std::vector<Expression>{
            Expression(BinaryOperation(var_exp, BinOpEnum::ADD, step_exp, le_range))},
        le_range);
    auto zero_exp = Expression(Literal(LiteralType::NUMBER, "0", le_range));
    auto if_cond_left = Expression(BinaryOperation(
        Expression(BinaryOperation(step_exp, BinOpEnum::GEQ, zero_exp, le_range)), BinOpEnum::AND,
        Expression(BinaryOperation(var_exp, BinOpEnum::GT, limit_exp, le_range)), le_range));
    auto if_cond_right = Expression(BinaryOperation(
        Expression(BinaryOperation(step_exp, BinOpEnum::LT, zero_exp, le_range)), BinOpEnum::AND,
    ...
    /*W2*/ auto if_statement2 = IfStatement(
        Expression(BinaryOperation(if_cond_left, BinOpEnum::OR, if_cond_right, le_range)),
    ...
    /*4*/ auto while_loop = WhileStatement(
        Expression(Literal(LiteralType::TRUE, "true", le_range)), while_body, le_range);
    Body do_body = Body(
...
auto identifier_vector_to_variable_declarator(std::vector<Identifier> id_vec, minilua::Range range)
    ...
    if (id_vec.empty()) {
        ...
        auto current_fe = FieldExpression(Prefix(VariableDeclarator(id_vec[0])), id_vec[1], range);
        while (iterator != id_vec.end()) {
            current_fe = FieldExpression(Prefix(VariableDeclarator(current_fe)), *iterator, range);
        }
...
auto FunctionStatement::desugar() const -> VariableDeclaration {
    ...
    if (method_optional.has_value()) {
        ...
        return VariableDeclaration(
            ...
            std::vector<Expression>{Expression(fd)}, this->range());
    } else {
        FunctionDefinition fd = FunctionDefinition(this->parameters(), this->body(), this->range());
        return VariableDeclaration(
            this->local(), std::vector<VariableDeclarator>{declarator},
...
auto ForInStatement::desugar() const -> DoStatement {
    ...
    FunctionCall fc = FunctionCall(
        Prefix(VariableDeclarator(f)), nullopt,
        std::vector<Expression>{Expression(s), Expression(var)}, le.range());
    ...
    /*W3*/ VariableDeclaration vd_3 = VariableDeclaration(
        ...
        std::vector<Expression>{Expression(ids[0])}, le.range());
    auto for_statements = this->body().statements();
    auto while_statements =
    ...
    auto do_body = Body(std::vector<Statement>{Statement(vd_1), Statement(while_loop)}, nullopt);
    return DoStatement(do_body, this->range());
}
